...
SingPromfter는 Flutter 클라이언트 단독 앱이다. 별도 서버나 인증 계층 없이 로컬 파일 시스템과 SharedPreferences에 데이터를 저장한다. Sprint 3 이후 메인 화면은 서비스·코디네이터·패널 위젯으로 분리되었고, v1.1.x에서 상단 탭 UI·키보드 포커스 범위(`PrompterKeyboardScope`)가 추가되었다.
...
flowchart TD
  ...
  Main --> View["SongListScreenView"]
  View --> TopNav["AppTopNavBar"]
  View --> Panels["곡목록 / PrompterPanel / Queue"]
  Main --> Keyboard["PrompterKeyboardScope"]
...
- 메인 화면: `SongListScreen` → `SongListScreenContent` → `SongListScreenView`
- 프롬프터 화면: `PrompterScreen`
- 키보드 범위: `PrompterKeyboardScope` (Shortcuts + Actions + Focus)
- 상태 관리: `StatefulWidget` + 서비스 계층
...
- 굵게 표시
- 저시력/원거리 프리셋
- 50dp 터치 타겟 (`AppConstants.minTouchTarget`), Semantics 라벨, 보조 +/- 버튼
- 키보드 단축키: Space, F5, Escape, ↑↓ 볼륨, ←→ 프롬프터 속도 (`PrompterKeyboardScope`)
